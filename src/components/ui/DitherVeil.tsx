...
const DEFAULT_SRC = 'https://images.unsplash.com/photo-1518770660439-4636190af475?q=80&w=1400&auto=format&fit=crop';
...
interface InternalSettings extends DitherVeilProps {
  fit: 'contain' | 'cover';
  pattern: 'bayer' | 'noise' | 'atkinson' | 'floyd' | 'lines';
  pixelSize: number;
  levels: number;
...
export const DitherVeil: React.FC<DitherVeilProps> = ({
  ...
  useEffect(() => {
    ...
    const viewUniforms: Record<string, { value: any }> = {
    ...
    const program = (fragment: string, uniforms: Record<string, { value: any }>) =>
    ...
    const updateDiffusion = (cell: number, s: InternalSettings) => {

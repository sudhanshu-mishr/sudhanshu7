...
const smoothFade = (t: number) => t * t * t * (t * (t * 6 - 15) + 10);

const gradDot = (h: number, x: number, y: number, z: number) => {
  const u = h < 8 ? x : y;
  const v = h < 4 ? y : h === 12 || h === 14 ? x : z;
  return ((h & 1) === 0 ? u : -u) + ((h & 2) === 0 ? v : -v);
};

const noise3 = (perm: Uint16Array, x: number, y: number, z: number) => {
...
export const SwarmCursor: React.FC<SwarmCursorProps> = ({
  color = '#ffffff',
  accentColor = '#ffffff',
  count = 10,
  size = 10,
  ...
  useEffect(() => {
    ...
    // Also attach window listeners so that when used as a full-screen overlay or wrapper, pointer movement is reliably captured across all clickable children
    ...
    const frame = (now: number) => {
      ...
      renderer.render({ scene: fieldMesh, target: target || undefined, clear: true });
      compProgram.uniforms.tField.value = target ? (target as any).texture : null;
      renderer.render({ scene: compMesh });
    };
    raf = requestAnimationFrame(frame);
